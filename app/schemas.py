...
class CategoryCreate(BaseModel):
    name: str = Field(
        min_length=2, max_length=50, description="Название категории (3-50 символов)"
    )
...
class Category(BaseModel):
    id: int = Field("Уникальный идентификатор категории")
    name: str = Field(description="Название категории")
    parent_id: Optional[int] = Field(None, description="ID родительской категории")
    is_active: bool = Field(Field(description="Активность категории"))
...
class ProductCreate(BaseModel):
    ...
    image_url: Optional[str] = Field(
        None, max_length=200, description="URL изображения"
    )
...
class ReviewCreate(BaseModel):
    ...
    comment: str = Field(default=None, description="Отзыв")
    grade: int = Field(ge=1, le=5, description="Оценка пользователя (от 1 до 5)")
...
class CartItem(BaseModel):
    ...
    product: ProductSheme = Field(..., description="Информация о товаре")

    model_config = ConfigDict(from_attributes=True)


class Cart(BaseModel):
    user_id: int = Field(..., description="Информация о пользователе")
